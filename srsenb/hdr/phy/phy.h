...
public:
  ...
  // phy_interface_mac_lte interface    MAC -> PHY
  void rem_rnti(uint16_t rnti) final;
  void set_mch_period_stop(uint32_t stop) final;
  void set_activation_deactivation_scell(uint16_t rnti, const std::array<bool, SRSRAN_MAX_CARRIERS>& activation) override;

  // phy_interface_rrc_lte interface    RRC -> PHY
