...
#include "srsenb/hdr/stack/funsplit/fs_utils.h" // For the different roles and functional split types.
...
typedef struct {
  uint8_t     role;                // For the functional split deployment...
  uint8_t     functionalSplitType; // For the functional deployment
  fs_ports_t  ports;
  std::string localIP;
  std::string remoteIP;
} func_split_t;
