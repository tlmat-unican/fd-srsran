 ...
 * \endcode
 */
#define LOG_REGISTER_MODULE(name) static LogModule s_LogModule(name);
 ...
 * \encode
 */
#define LOG_SET_LEVEL(name, level) LogManager::Instance().SetLogLevel(name, level);
 ...
 * variable
 */
#define LOG_SET_ALL_LEVEL(level) LogManager::Instance().SetLogLevel(level);
...
#define BEG                                                                                                            \
  if (s_LogModule.GetLevel() == LogLevel::ALL)                                                                         \
    LogPrint(WHITE, __TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [IN]      ", __METHOD_NAME__, "\n");
...
#define END                                                                                                            \
  if (s_LogModule.GetLevel() == LogLevel::ALL)                                                                         \
    LogPrint(WHITE, __TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [OUT]     ", __METHOD_NAME__, "\n");
...
#define BEGEND                                                                                                         \
  if (s_LogModule.GetLevel() == LogLevel::ALL)                                                                         \
    LogPrint(__TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [IN-OUT]  ", __METHOD_NAME__, "\n");
...
#define DBG(...)                                                                                                       \
  if (s_LogModule.GetLevel() >= LogLevel::DEBUG)                                                                       \
    LogPrint(BOLDGREEN, __TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [DEBUG]   ==> ", __VA_ARGS__, "\n");
...
#define INFO(...)                                                                                                      \
  if (s_LogModule.GetLevel() >= LogLevel::INFO)                                                                        \
    LogPrint(BOLDCYAN, __TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [INFO]    ==> ", __VA_ARGS__, "\n");
...
#define WARN(...)                                                                                                      \
  if (s_LogModule.GetLevel() >= LogLevel::WARNING) {                                                                   \
    if (!LogRedirected) {                                                                                              \
      LogPrint(                                                                                                        \
          WARNING_COL, __TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : ", "[WARNING] ==> ", __VA_ARGS__, RESET, "\n"); \
      LogPrint("");                                                                                                    \
    } else {                                                                                                           \
      LogPrint(__TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [WARNING] ==> ", __VA_ARGS__, "\n");                    \
    }                                                                                                                  \
  }
...
#define ERROR(...)                                                                                                     \
  if (s_LogModule.GetLevel() >= LogLevel::ERROR) {                                                                     \
    if (!LogRedirected) {                                                                                              \
      LogPrint(                                                                                                        \
          ERROR_COL, __TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : ", "[ERROR]   ==> ", __VA_ARGS__, RESET, "\n");   \
    } else {                                                                                                           \
      LogPrint(__TIME_INFO__ __FILE_INFO__ __MODULE_NAME__ " : [ERROR]   ==> ", __VA_ARGS__, "\n");                    \
    }                                                                                                                  \
  }
...
#define MSG_ASSERT(expr, ...)                                                                                          \
  if (!(expr)) {                                                                                                       \
    LogPrint(ASSERT_COL, __TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [ASSERT]  ==> ", __VA_ARGS__, RESET, "\n");   \
...
#define UBEG                                                                                                           \
  if (!LogRedirected)                                                                                                  \
    LogPrint(                                                                                                          \
        HEADERS_COL, __TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [IN]      ", __FUNCTION__, RESET, RESET, "\n");   \
  else                                                                                                                 \
    LogPrint(__TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [IN]      ", __FUNCTION__, "\n");
...
#define UEND                                                                                                           \
  if (!LogRedirected)                                                                                                  \
    LogPrint(                                                                                                          \
        HEADERS_COL, __TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [OUT]     ", __FUNCTION__, RESET, RESET, "\n");   \
  else                                                                                                                 \
    LogPrint(__TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [OUT]     ", __FUNCTION__, "\n");
...
#define UINFO(...)                                                                                                     \
  if (!LogRedirected)                                                                                                  \
    LogPrint(                                                                                                          \
        HEADERS_COL, __TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [INFO]   ==> ", __VA_ARGS__, RESET, RESET, "\n"); \
  else                                                                                                                 \
    LogPrint(__TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [INFO]   ==> ", __VA_ARGS__, "\n");
...
#define UWARN(...)                                                                                                     \
  if (!LogRedirected)                                                                                                  \
    ...
    LogPrint(__TIME_INFO__ "[", __FILE__, ": ", __LINE__, "] [WARNING] ==> ", __VA_ARGS__, "\n");
...
#define UERROR(...)                                                                                                    \
  if (!LogRedirected)                                                                                                  \
    ...
    LogPrint(__FILE_INFO__ "[", __FILE__, ": ", __LINE__, "] [ERROR]    ==> ", __VA_ARGS__, "\n");
