...
namespace srsenb
{
  ...
  int enb::init(const all_args_t &args_)
  {
    ...
    if (args.stack.type == "lte")
    {
      ...
      if (args.stack.func_split.functionalSplitType == EnumValue(FUNCTIONAL_SPLIT::NONE))
      {
        ...
        stack_phy_proxy *stack_phy = new stack_phy_proxy();
        srsenb::mac_phy_wrapper *mac_phy_wrapper = new srsenb::mac_phy_wrapper();
        srsenb::rrc_phy_wrapper *rrc_phy_wrapper = new srsenb::rrc_phy_wrapper();
        stack_phy->init(mac_phy_wrapper, rrc_phy_wrapper);

        rrc_phy_wrapper->init(lte_phy.get(),
                              FsConf_e::LOCAL,
                              args.stack.func_split.localIP,
                              args.stack.func_split.remoteIP,
                              args.stack.func_split.ports.rrc_phy,
                              args.stack.func_split.ports.phy_rrc);

        mac_phy_wrapper->init(lte_stack.get(),
                              lte_phy.get(),
                              FsConf_e::LOCAL,
                              args.stack.func_split.localIP,
                              args.stack.func_split.remoteIP,
                              args.stack.func_split.ports.mac_phy,
                              args.stack.func_split.ports.phy_mac);

        if (lte_phy->init(args.phy, phy_cfg, lte_radio.get(), stack_phy))
        {
          ret = SRSRAN_ERROR;
        }
      ...
      {
        if (args.stack.func_split.role == EnumValue(ROLES::DU))
        {
          std::unique_ptr<srsran::radio> lte_radio = std::unique_ptr<srsran::radio>(new srsran::radio);
          if (!lte_radio)
          {
          ...
          mac_phy_wrapper->init(lte_stack.get(),
                                lte_phy.get(),
                                (args.stack.func_split.functionalSplitType == EnumValue(FUNCTIONAL_SPLIT::MAC_PHY_SPLIT)
                                     ? FsConf_e::REMOTE
                                     : FsConf_e::LOCAL),
                                args.stack.func_split.localIP,
                                args.stack.func_split.remoteIP,
        ...
        else if ((args.stack.func_split.role == EnumValue(ROLES::CU)))
        {
          if (lte_stack->init(args.stack, rrc_cfg, lte_phy.get()) != SRSRAN_SUCCESS)
          {
            return SRSRAN_ERROR;
