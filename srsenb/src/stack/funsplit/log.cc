...
//#include <cassert>
#include "srsenb/hdr/stack/funsplit/log.h"
...
{
  auto check_ = m_modules.insert(ModulesMap_t::value_type{logModule.GetModuleName(), logModule});
...
LogModule::LogModule(std::string name, LogLevel level) : m_name(name), m_level(level)
{
  CheckEnvVar();
...
void LogModule::CheckEnvVar(void)
{
  ...
  logVar_ =
      token_ == std::string::npos ? logVar_.substr(beg_, logVar_.size() - beg_) : logVar_.substr(beg_, token_ - beg_);
...
std::string LogModule::GetModuleName(void) const
{
  return m_name;
}

std::string LogModule::GetPrintName(void) const
{
...
LogLevel LogModule::GetLevel(void) const
{
  return m_level;
}