...
int mac::get_dl_sched(uint32_t tti_tx_dl, dl_sched_list_t& dl_sched_res_list)
{
  ...
  for (uint32_t enb_cc_idx = 0; enb_cc_idx < cell_config.size(); enb_cc_idx++) {
        ...
        if (ue_db.contains(rnti)) {
          ...
          for (uint32_t tb = 0; tb < SRSRAN_MAX_TB; tb++) {
            ...
            if (sched_result.data[i].nof_pdu_elems[tb] > 0) {
              ...
              dl_sched_res->pdsch[n].data_len[tb] = sched_result.data[i].tbs[tb];
    ...
    for (uint32_t i = 0; i < sched_result.rar.size(); i++) {
      ...
      // Assemble PDU
      dl_sched_res->pdsch[n].data_len[0] = sched_result.rar[i].tbs;
    ...
    for (uint32_t i = 0; i < sched_result.bc.size(); i++) {
      ...
      if (sched_result.bc[i].type == sched_interface::dl_sched_bc_t::BCCH) {
        ...
        dl_sched_res->pdsch[n].data_len[0] = SRSRAN_MAX_BUFFER_SIZE_BYTES - SRSRAN_BUFFER_HEADER_OFFSET;
        ...
        dl_sched_res->pdsch[n].data_len[0]      = pcch_payload_buffer_len;
...
int mac::get_ul_sched(uint32_t tti_tx_ul, ul_sched_list_t& ul_sched_res_list)
{
  ...
  for (uint32_t enb_cc_idx = 0; enb_cc_idx < cell_config.size(); enb_cc_idx++) {
...

    {
      ...
      int n                        = 0;
      // std::cout << "MAC - No. grants " << sched_result.pusch.size() << std::endl;
      for (uint32_t i = 0; i < sched_result.pusch.size(); i++) {
        if (sched_result.pusch[i].tbs > 0) {
          // std::cout << "MAC - Grants " << i << std::endl;
          // Get UE
          uint16_t rnti = sched_result.pusch[i].dci.rnti;

          if (ue_db.contains(rnti)) {
            // Copy grant info
            // std::cout << "MAC - UE RNTI exists: Copying grant info at " << n << std::endl;
            ...
            if (sched_result.pusch[n].current_tx_nb == 0) {
              // std::cout << "MAC - Reseting soft buffer at " << n << std::endl;
            ...
            if (phy_ul_sched_res->pusch[n].data) {
              phy_ul_sched_res->pusch[n].data_len = sched_result.pusch[i].tbs;
              // std::cout << "MAC - Setting data at " << n << " sith datalent " << phy_ul_sched_res->pusch[n].data_len
              // << std::endl;
              phy_ul_sched_res->nof_grants++;
            } else {
              // std::cout << "MAC - Grant for rnti " << rnti << " could not be allocated due to lack of buffers"
              // << std::endl;
              logger.error("Grant for rnti=0x%x could not be allocated due to lack of buffers", rnti);
            }
            n++;
          } else {
            // std::cout << "MAC - Invalid UL scheduling result.User " << rnti << " does not exist " << std::endl;
            logger.warning("Invalid UL scheduling result. User 0x%x does not exist", rnti);
          }

        } else {
          // std::cout << "MAC - Grant " << i << " for rnti=" << sched_result.pusch[i].dci.rnti << "has zero TBS"
          //           << std::endl;
    ...
    // Copy PHICH actions
    // std::cout << "MAC - No. phcih " << sched_result.phich.size() << std::endl;
    for (uint32_t i = 0; i < sched_result.phich.size(); i++) {
      // std::cout << "MAC - Copying phcih " << i << std::endl;
