...
namespace srsenb
{
  ...
  rrc::rrc(srsran::task_sched_handle task_sched_) : logger(srslog::fetch_basic_logger("RRC")), task_sched(task_sched_), rx_pdu_queue(64)
  {
  }
  ...
  *******************************************************************************/
  ...
  uint8_t *rrc::read_pdu_bcch_dlsch(const uint8_t cc_idx, const uint32_t sib_index)
  {
    ...
    tti_clock(); // UC: this has been added and should be removed
    if (sib_index < ASN1_RRC_MAX_SIB && cc_idx < cell_common_list->nof_cells())
    {
      // std::cout << "rrc::read_pdu_bcch_dlsch << The size msg is "
      //           << cell_common_list->get_cc_idx(cc_idx)->sib_buffer.at(sib_index)->size() << "/ max "
      //           << SRSRAN_MAX_BUFFER_SIZE_BYTES << " / header " << SRSRAN_BUFFER_HEADER_OFFSET << " / diff "
      //           << SRSRAN_MAX_BUFFER_SIZE_BYTES - SRSRAN_BUFFER_HEADER_OFFSET << std::endl;
